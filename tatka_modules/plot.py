...
#--plotting horizontal projection of the stacked grid----------------------
    ...
    for sta in coord_sta:
        ...
        trans = ax1.transData + ax1.transAxes.inverted()
        x_sta_ax, y_sta_ax = trans.transform((x_sta, y_sta))
        ax1.text(x_sta_ax+0.02, y_sta_ax+0.02, sta, fontsize=12, color='k', transform=ax1.transAxes)
    ...
    for sta in coord_sta:
        ...
        trans = ax11.transData + ax11.transAxes.inverted()
        x_sta_ax, y_sta_ax = trans.transform((x_sta, y_sta))
        ax11.text(x_sta_ax+0.02, y_sta_ax+0.02, sta, fontsize=12, color='k', transform=ax11.transAxes)
    if grid_max[x_max,y_max,z_max] >= LTrig:
...
#--plotting vertical projections of the stacked grid
    ...
    ax4 = fig.add_axes([0.545,0.44,0.40,0.54])
    ax4.set_xlim(0, max(time))
    sta_y = [coord_sta[sta][1] for sta in coord_sta]
    ax4.set_ylim(min(sta_y), max(sta_y))
    ax4.set_xlabel('Time[sec]')
    ax4.set_ylabel('Y[km]')
    labels = ax4.get_yticklabels()
    pylab.setp(labels, rotation=90, fontsize=12)
    trans = ax4.transData + ax4.transAxes.inverted()
    ...
    for Record, CH_fct in zip(st, st_CF):
        ...
        if plot_waveforms:
            ...
            ax4.plot(time, ydata, 'k', alpha=0.4, rasterized=True)
        # Project signal to Axes coordinates:
        signal = CH_fct.data/abs(CH_fct.max())*0.05 + y_sta_ax
        xydata = np.dstack((np.zeros_like(signal), signal))[0]
        ydata = invtrans.transform(xydata)[:,1]
        ax4.plot(time_env, ydata, 'k', rasterized=True)
    ...
    ax4.set_title(note_t, fontsize=15)
    if len(grid_max[grid_max >= LTrig]) > 1:
...
def plt_SummaryOut(st_CF, st, plot_waveforms, ch_function,time_env, time, coord_sta,
    ...
    ax4 = fig.add_axes([0.05,0.472,0.85,0.51])
    ax4.set_xlim(0,max(time))
    sta_y = [coord_sta[sta][1] for sta in coord_sta]
    ax4.set_ylim(min(sta_y), max(sta_y))
    ax4.set_xlabel('Time[sec]')
    ...
    ax4.set_title(note, fontsize=15)
