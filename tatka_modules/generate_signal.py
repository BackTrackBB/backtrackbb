...
def generate_signal_expSin(position1,decreasingFactor1,pulse1,noise,amplitude2=1,position2=0,decreasingFactor2=0,pulse2=0):
    signal=noise.copy()

    for i  in range(position1,len(noise)-1):
        signal[i-1]=signal[i-1] + function_sinExp(i-position1,decreasingFactor1,pulse1)
    for i in range(position2,len(noise)-1):
        signal[i-1]=signal[i-1] + amplitude2*function_sinExp(i-position2,decreasingFactor2,pulse2)
