...
import numpy as np
from backtrackbb.bp_types import Pick
...
def TrOrig_time(config, GRD_sta, trigger, arrival_times):
    ...
    for sta, wave in ((sta, wave) for wave in config.wave_type
                      for sta in config.stations):
        pick = Pick()
        pick.station = sta
        pick.arrival_type = wave
        pick.travel_time =\
            GRD_sta[sta][wave].get_value(trigger.x, trigger.y, trigger.z)
        try:
    ...
    if config.grid_type != config.wave_type:
        ...
        for sta in config.stations:
            ...
            pick.theor_time =\
                GRD_sta[sta][phase2].get_value(trigger.x, trigger.y, trigger.z)
            pick.pick_time = -10.0
            picks.append(pick)
            trigger.add_pick(pick)
